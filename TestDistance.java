...
public class TestDistance {
    ...
    static final double CONTACT_ADJ = 0.85;
    static final double DIST_COEF = 0.35;
    static final double POWER_MAX = 45.0;
    static final double DIST_THRESHOLD = 6.1; // 대략 공 지름
    static final double SCORE_PENALTY = -10000.0;
    static final double COS_THREASHOLD = 0.75;
    static final double COS_PRIORITY = 5000.0;
    static final double COS_COEF = 200.0;
    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            while (socket != null) {
                ...
                int currBall = myBalls.get(0);
                double[] targetHole = { HOLES[0][0], HOLES[0][1] };
                double maxScore = SCORE_PENALTY * 10;
                for (int ballIdx : myBalls) {
                    ...
                    for (int[] hole : HOLES) {
                        ...
                        // 코사인 값이 높을수록 각도가 유사
                        double cosineOverThreashold = cos > COS_THREASHOLD ? 1 : 0;

                        double[] holeDoubled = { hole[0], hole[1]};
                        double[] contactPoint = findContactPoint(targetBall, holeDoubled);
                        double holeDist = distance(targetBall, holeDoubled);
                        double currScore = cos * COS_COEF + COS_PRIORITY * cosineOverThreashold - holeDist;

                        for (int otherBallIdx = 1; otherBallIdx < HOLES.length; otherBallIdx++) {
                            if (!myBalls.contains(otherBallIdx)) {
                                double[] otherBall = { balls[otherBallIdx][0], balls[otherBallIdx][1] };
                                double dist = distanceToLine(whiteBall, contactPoint, otherBall);
                                currScore += dist > DIST_THRESHOLD ? SCORE_PENALTY : 0;
                            }
                        }
                ...
                System.out.println(myBalls);
                System.out.println(currBall);
    ...
    static double vectorLength(double[] vec) { 
    ...
    static double distanceToLine(double[] lineStart, double[] lineEnd, double[] queryPoint) {
        double[] lineVector = {lineEnd[0] - lineStart[0], lineEnd[1] - lineStart[1]};
        double[] pointVector = {queryPoint[0] - lineStart[0], queryPoint[1] - lineStart[1]};
        double dotProduct = lineVector[0] * pointVector[0] + lineVector[1] * pointVector[1];
        
        double h = vectorLength(pointVector);
        double x = dotProduct / (vectorLength(lineVector) + EPS);
        if (x < 0 || x > vectorLength(lineVector)) {
            return h;
        }
    
        double dist = Math.sqrt(h * h - x * x);
        return dist;
    }
}
